...
class EventsIndex extends React.Component {
  ...
  searchEventsNoLocation = async (word, minDate) => {
    try {
      const { data } = await axios.get(`https://www.skiddle.com/api/v1/events/search/?api_key=${apiKey}&keyword=${word}&minDate=${minDate}&limit=100&order=distance&description=1`)
  ...
  searchEventsWithLocation = async (word, minDate, longitude, latitude) => {
    console.log(typeof longitude, typeof minDate)
    try {
      const { data } = await axios.get(`https://www.skiddle.com/api/v1/events/search/?api_key=${apiKey}&keyword=${word}&minDate=${minDate}&latitude=${latitude}&longitude=${longitude}&radius=10&limit=100&order=distance&description=1`)
      this.setState({ eventSearchResult: data.results })
      this.checkSearch(data)
    } catch (err) {
      console.log(err)
